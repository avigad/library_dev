...
() : unit ← ↑(@guard tactic _ ((qf↣1↣get_lit i)↣is_neg) _),
h ← ↑(mk_local_def `h (qf↣1↣get_lit i)↣formula↣not_),
new_hyps ← f h,
return $ new_hyps↣for $ λnew_hyp,
  (((clause.mk 0 0 new_hyp↣2 false_)↣close_const h)↣inst (op↣1↣close_const lci)↣proof)↣close_constn
  (qf↣2 ++ op↣2↣remove i ++ new_hyp↣1)
