...
-/
import .bounded_lattice_experiment data.set
...
@[simp] lemma set_of_false : {a : α | false} = ∅ := set.ext $ take a, by simp [mem_empty_eq]
...
@[simp] theorem subset_insert (x : α) (s : set α) : s ⊆ insert x s := take y, assume ys, or.inr ys
